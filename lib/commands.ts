		...
		threads.push(new Promise<void>(async (res, reject) => {
			try {
				while (queue.length > 0) {
					...
					task.state = 'running';
					task.start_timer();
					await service.up(label => message => {
task_list.log({label, message});});
...
export async function do_down(store: Store): Promise<void> {
	...
	for (const service of services) {
		threads.push((async () => {
			...
			task.state = 'running';
			task.start_timer();
			await service.down(label => message => {
task_list.log({label, message});});
			task.completed = true;
			task.state = 'successful';
...
export async function do_pull(store: Store): Promise<void> {
	...
	for (const buildable_service of buildable) {
		...
		tasks.push(exec_command(
			...
			(message: string) => {
task_list.log({label: buildable_service.name, message});},
