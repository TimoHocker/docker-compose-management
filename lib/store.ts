...
export class Store {
	...
	private async read_volumes(): Promise<void> {
		...
		assert.ok(Array.isArray(data), 'volumes.json is not an array');
		this.volumes = [];
	...
	private async read_networks(): Promise<void> {
		...
		assert.ok(Array.isArray(data), 'networks.json is not an array');
		this.networks = [];
	...
	private async read_services(): Promise<void> {
		...
		assert.ok(Array.isArray(passive), 'passive.json is not an array');
		const dependencies = await this.read_json('./dependencies.json');
		log('Dependencies:', dependencies);
		assert.strictEqual(typeof dependencies, 'object', 'dependencies.json is not an object');
		...
		for (const file of list) {
			...
			try {
				...
				console.warn(`${file}: error reading docker-compose.yml: ${error}`);
				continue;
			}
			...
			const contents = YAML.parse(await fs.readFile(service.compose_file, 'utf8'));
